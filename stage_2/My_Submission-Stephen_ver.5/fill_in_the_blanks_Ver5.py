...
def wrong_answer(num_of_try, blank_set, blank_sol, test_set):
    ...
    '''
    min_of_try = 0
    if num_of_try >= min_of_try and num_of_try < chances-2:
...
def problem_set(blank_set, blank_sol, test_set):
    ...
    while index < chances:
        ...
        while num_of_try < chances:
            solution1_input = raw_input(test_set + '\n \nPlease fill in the blank ' + str(index+1) +': ')
            if solution1_input == blank_sol[index]:
                print '\nYour solution is correct ' + user_name +'!\n'
                total_score += socre_for_answer
                test_set = test_set.replace(blank_set[index],blank_sol[index])
                break
            else:
                wrong_answer(num_of_try, blank_set[index], blank_sol[index], test_set)
                num_of_try += 1
        index += 1
...
def choose_level():
    ...
    if level in ['easy','medium','hard']:#== 'easy' or level=='medium' or level=='hard':
