...
def wrong_answer(num_of_try, p_set, p_sol):
    ...
    if num_of_try >= 0 and num_of_try<2:
        ...
        print 'Study hard '+user_name[0]+'.'
        test_set = test_set.replace(p_set,p_sol)

def solution_input(t, p_set, p_sol):
    # take input as answer and if it is match with solution return filled text
    num_of_try = 0
    global test_set
    global total_score
    while num_of_try < chances:
        ...
        if solution1_input == p_sol:
            print '\nYour solution is correct ' + user_name[0]+'!\n'
            total_score += 25
            test_set = test_set.replace(p_set,p_sol)
            break
        else:
...
def choose_level():
    ...
    global pp_sol = game_data[level]['answer']
    global test_set = game_data[level]['quiz']
    # if level == 'easy':
    #     pp_sol = game_data[level]
    # elif level == 'math':
    #     return math_problem()
    # else:
...
def start_fbgame():
    ...
    print user_name[0] + ', your score is ' + str(total_score)
